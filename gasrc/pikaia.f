...
c     Constants
      ...
      parameter (NMAX = 32, PMAX = 2024, DMAX = 6)
c
...
!!!初始化数组的值  
c            call random_seed()
c            call random_number(oldph(k,ip))         
            oldph(k,ip)=urand()
         end do
      end do
...
c     write each gen's best fitness
         write(*,*)'gen:',ig-1,'f:',fitns(ifit(np))
         if(mod(ig-1,100).eq.0)then
            do k=1,n
                   x(k)=xbound(1,k)+dble(oldph(k,ifit(np)))*
     +             (xbound(2,k)-xbound(1,k))
                   write(*,"(f10.6)") x(k)
            end do 
         end if
         newtot=0
!!!c!$omp parallel do private(phasetheory,ip1,ip2,gn1,gn2,ph)    
         do 20 ip=1,np/2
...
c     Local
      ...
      real     DFAULT(12)
      save     DFAULT
      data     DFAULT /100,500,5,.85,2,.01,.0005,.3,1,1,1,0/
c
      do 1 i=1,12
